        ...
        case startRank of
          ...
          rank -> PawnMoves whiteMoves blackMoves
            where startFile = _file start
                  forwardWhite =
                    Just $ Move start (Square startFile (succ startRank))
                  forwardBlack =
                    Just $ Move start (Square startFile (pred startRank))
                  jumpWhite =
                    case startRank of
                      R2 ->
                        Just $
                        Move start (Square startFile (succ $ succ startRank))
                      _ -> Nothing
                  jumpBlack =
                    case startRank of
                      R7 ->
                        Just $
                        Move start (Square startFile (pred $ pred startRank))
                      _ -> Nothing
                  takeLeftWhite =
                    case startFile of
                      Fa -> Nothing
                      _ ->
                        Just $
                        Move start (Square (pred startFile) (succ startRank))
                  takeLeftBlack =
                    case startFile of
                      Fa -> Nothing
                      _ ->
                        Just $
                        Move start (Square (pred startFile) (pred startRank))
                  takeRightWhite =
                    case startFile of
                      Fh -> Nothing
                      _ ->
                        Just $
                        Move start (Square (succ startFile) (succ startRank))
                  takeRightBlack =
                    case startFile of
                      Fh -> Nothing
                      _ ->
                        Just $
                        Move start (Square (succ startFile) (pred startRank))
                  enPassentLeftWhite =
                    case startRank of
                      R5 ->
                        fmap
                          (\tl ->
                             EnPassent tl (Square (view (end . file) tl) R5))
                          takeLeftWhite
                      _ -> Nothing
                  enPassentLeftBlack =
                    case startRank of
                      R4 ->
                        fmap
                          (\tl ->
                             EnPassent tl (Square (view (end . file) tl) R4))
                          takeLeftBlack
                      _ -> Nothing
                  enPassentRightWhite =
                    case startRank of
                  ...
                  enPassentsWhite =
                    maybesToList enPassentLeftWhite enPassentRightWhite
                  enPassentsBlack =
                    maybesToList enPassentLeftBlack enPassentRightBlack
                  whiteMoves =
                    case startRank of
                      R7 ->
                        PM
